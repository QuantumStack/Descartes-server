 ...
 */
const studentEnrollWithCode = (user, courseUUID) =>
  user.$relatedQuery('profile').then(profile =>
    Course.query()
      .findOne('uuid', courseUUID)
      .then(course => {
        if (!course) {
...
router.post('/', (req, res) => {
  ...
  return studentEnrollWithCode(req.user, code).then(err => {
    if (err) {
      return res.status(400).json({
        success: false,
        error: err.name,
